...
def get_session_file_path():
    return os.path.join(_sticky_dir(), ".sticky-session")


def save_session_id(session_id):
    """Persist a generated session ID so all hooks in this session share it."""
...
def read_session_id_from_file():
    """Read the file-based session ID if it exists."""
    try:
        with open(get_session_file_path(), "r", encoding="utf-8") as f:
            sid = f.read().strip()
...
def clear_session_file():
    """Remove the .sticky-session file at session end."""
    try:
...
def get_session_id(hook_input=None):
    """Extract session ID from hook input, env vars, file, or generate a fallback."""
    if hook_input:
    ...
    for env_key in ("SESSION_ID", "GITHUB_COPILOT_SESSION_ID", "WT_SESSION"):
        ...
        if val:
            return val
    # Fall back to file-based session ID (generated by session-start.py)
    file_sid = read_session_id_from_file()
    if file_sid:
        return file_sid
    return "unknown"
